...
namespace Logistic
{
    public partial class employee_DeliveryPerforma : System.Web.UI.Page
    {
        ...
        protected void gridState_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Edit1")
            {
                ...
                if (dt.Rows.Count != 0)
                {
                    ...
                    //  txtConsignementID.Text = dt.Rows[0]["ConsignementId"].ToString();
        ...
        {
            DataTable dt = new DataManager().SelectConsignmentByConsignementID1(ddConsignementID.SelectedItem.Text);
            if (dt.Rows.Count > 0)
            {
                ddConsignementID.SelectedValue = dt.Rows[0]["Id"].ToString();
                txtAddress1.Value = dt.Rows[0]["Address1"].ToString();
                txtConsignee2.Value = dt.Rows[0]["ConsigneeName"].ToString();

                txtDate.Value = dt.Rows[0]["Date"].ToString();

                txtConsigner1.Value = dt.Rows[0]["stateName"].ToString();
