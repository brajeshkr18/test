...
namespace Logistic
{
    public partial class partner_Today_Vehicle_Request : System.Web.UI.Page
    {
        ...
        public void BindDataList()
        {
            ...
            DataTable data = new DataManager().SelectVehicleRequestbyPartnerIdToday(Convert.ToInt32(Session["PartnerId"]), today);
        ...
        protected void btnsubmit_Click1(object sender, EventArgs e)
        {
            if (btnsubmit.Text == "Update")
            {
                ...
                if (new DataManager().InsertUpdateVehicleRequest(Convert.ToInt32(lblId.Text), 1, txtFrom.Value.ToString(), txtTo.Value.ToString(), Convert.ToInt32(ddlType.SelectedValue), ddlTop.SelectedValue, ddlWeight.SelectedValue.ToString(), txtFreight.Value.ToString(), txtComments.Value.ToString(), ddlStatus.SelectedValue, Convert.ToInt32(Session["PartnerId"]), ddlCity.SelectedValue, "", "", "") > 0)
                {
                    Clear();
                    btnsubmit.Text = "Submit";
                    MultiView1.ActiveViewIndex = 1;
            ...
            {
                if (new DataManager().InsertUpdateVehicleRequest(0, 0, txtFrom.Value.ToString(), txtTo.Value.ToString(), Convert.ToInt32(ddlType.SelectedValue), ddlTop.SelectedValue, ddlWeight.SelectedValue.ToString(), txtFreight.Value.ToString(), txtComments.Value.ToString(), ddlStatus.SelectedValue, Convert.ToInt32(Session["PartnerId"]), ddlCity.SelectedValue, "", "", "") > 0)
                {
                    Clear();
                    MultiView1.ActiveViewIndex = 1;
                    BindDataList();
        ...
        private void SearchText()
        {
            ...
            string today = DateTime.Now.ToString("yyyy-MM-dd");
            DataTable dt = new DataManager().SelectVehicleRequestbyPartnerIdToday(Convert.ToInt32(Session["PartnerId"]), today);
