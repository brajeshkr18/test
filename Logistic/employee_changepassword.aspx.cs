        ...
        protected void btnsubmit_Click1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (con.State == ConnectionState.Closed)
                    con.Open();
                cmd.CommandText = "select Password,Id from Core_Employee where Password='" + txtOld.Value + "'";
                cmd.Connection = con;
                ...
                if (dr.Read())
                {
                    ...
                    if (txtOld.Value == dr.GetString(0))
                    {
                        ...
                        if (new DataManager().UpdateEmployeePassword(Convert.ToInt32(id), txtNewPassword.Value.ToString()) > 0)
                        {
