...
namespace Logistic
{
    public partial class employee_OldConsignmentClient : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = new DataManager().SelectConsignmentbyStatusAndCircle("Delivered", Convert.ToInt32(Session["EmployeeId"]));
            if (dt.Rows.Count != 0)
            {
        ...
        protected void ddlConsignementID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = new DataManager().SelectConsignmentByConsignementID1(ddlConsignementID.SelectedItem.Text);
                //DataTable dt = new DataManager().SelectConsignmentByConsignementID(ddlConsignementID.SelectedValue);
                if (dt.Rows.Count > 0)
                {
