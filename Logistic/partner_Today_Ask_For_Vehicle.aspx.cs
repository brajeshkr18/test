...
namespace Logistic
{
    public partial class partner_Today_Ask_For_Vehicle : System.Web.UI.Page
    {
        ...
        public void BindDataList()
        {
            ...
            DataTable data = new DataManager().SelectAskForVehiclebyPartnerIdToday(Convert.ToInt32(Session["PartnerId"]), today);
        ...
        protected void btnsubmit_Click1(object sender, EventArgs e)
        {
            if (btnsubmit.Text == "Update")
            {
                ...
                if (new DataManager().InsertUpdateAskForVehicle(Convert.ToInt32(lblId.Text), 1, txtRequestNo.Value, txtFrom.Value.ToString(), txtTo.Value.ToString(), Convert.ToInt32(ddlType.SelectedValue), ddlTop.SelectedValue, txtWeight.Value.ToString(), txtFreight.Value.ToString(), txtComments.Value.ToString(), ddlStatus.SelectedValue, Convert.ToInt32(Session["PartnerId"]), ddlCity.SelectedValue) > 0)
                {
                    Clear();
                    btnsubmit.Text = "Submit";
                    MultiView1.ActiveViewIndex = 1;
        ...
        {
            string today = DateTime.Now.ToString("yyyy-MM-dd");
            DataTable dt = new DataManager().SelectAskForVehiclebyPartnerIdToday(Convert.ToInt32(Session["PartnerId"]), today);
            ...
            dv.RowFilter = "stateName LIKE " + SearchExpression + " OR RequestNo LIKE " + SearchExpression + "OR Type LIKE " + SearchExpression + " OR Tops like" + SearchExpression + "OR status like" + SearchExpression + "";
